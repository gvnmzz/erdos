...
subroutine erdos(a,n,p,c)
    ...
    ! The sing vector is initialised to one.
    !Every time node i gets a link, sing(i) is decreased by 1.
...
subroutine ludecpiv(a,n,c)
    ...
    do i=1,n
        ...
        ! Then we want to make sure that the diagonal element of
        ! U is the biggest in the i-th column
        ! Find the maximum in the i-th column of U.
        k = maxval(maxloc(abs(a(i:n,i))))+i-1

        if (abs(a(k,i)) < eps) then
        ! This means that we have a 0 on the diagonal of U,
        ! so the determinant is 0, so singular
            c = c+1
            return
        endif

        if (k /= i) call swap(a(i,:),a(k,:),n)
        ! Pivoting. No need to keep track because not solving system.
        ! Swapping rows
        ...
        do j=i+1,n
            ...
            enddo
            ! and divide the elements of the i-th column by the diagonal element
            a(j,i) = a(j,i)/a(i,i)
        enddo
